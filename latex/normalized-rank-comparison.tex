...
\begin{itemize}
    ...
    \item \textbf{$P_i$}: Normalized Performance Measure value in the range of [0, 1]
...
\subsubsection{Final Formula for unbiased (but non-normalized) Performance Measure}
...
The non-normalized formulae is normalized, for to satisfy these following criteria:
\begin{itemize}
...
\subsection{Derviation of Non-normalized Performance Measure}
...
We aim to minimize the non-normalized measure $p_i^{(non-normalized)}$ defined as:
\begin{equation}
	p_i^{(non-normalized)} = w_i \cdot \frac{\sum r_i}{ S_{UDH} }
\end{equation}
...
\begin{align*}
	p_i^{(non-normalized)} = \frac{8}{k(3N + 2)}\cdot \frac{\sum r_i}{n_i}
\end{align*}
...
The minima, depends on $\frac{\sum r_i}{n_i}$. And, for this to be low $\sum r_i$ should be low. That means $\sum r_i$ have to occupy the lowest ranks such as 1, 2, 3,... so on. We can rewrite $\frac{\sum r_i}{n_i}$ in this format below:

\begin{equation}
	 = \frac{\sum r_i}{n_i} = \frac{\frac{n_i}{2}(1+n_i)}{n_i} = \frac{1}{2} \cdot n_i + \frac{1}{2}
\end{equation}

As this is a linear increasing line equation with a positive slope ($y = mx + C$), minima of $\frac{\sum r_i}{n_i}$ is $1$ when $n_i = 1$, as $n_i$ is a natural number and can't be zero or negative, hence, substituting $n_i = 1$, we get, $\frac{\sum r_i}{n_i} = \frac{1}{2} + \frac{1}{2} = 1$, hence  $\frac{\sum r_i}{n_i} = 1 $

Therefore minima of Non-normalized measure is:
\begin{equation}
	p_i^{(min, \, non-normalized)} = \frac{8}{k(3N + 2)}
\end{equation}
